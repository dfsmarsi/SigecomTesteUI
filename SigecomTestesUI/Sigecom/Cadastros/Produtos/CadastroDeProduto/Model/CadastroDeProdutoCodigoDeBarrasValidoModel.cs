namespace SigecomTestesUI.Sigecom.Cadastros.Produtos.CadastroDeProduto.Model
{
    public class CadastroDeProdutoCodigoDeBarrasValidoModel
    {
        public static string NomeDoProduto => "PRODUTO CODIGO DE BARRAS CADASTRO TESTE";
        public static string CodigoDeBarras => "7896045506248";
        public static string OrigemMercadoria => "5 - Nacional, mercadoria ou bem com Conteúdo de Importação inferior ou igual a 40%";
        public static string SituacaoTributaria => "SUBSTITUIÇÃO TRIBUTÁRIA COBRADA ANTERIORMENTE";
        public static string NaturezaCfop => "Produto adquirido ou recebido de terceiros em operação com mercadoria sujeita ao regime ST, na condição de contribuinte substituído";
        public static string Ncm => "22030000";
        public static string Cest => "0302103";
        public static string Cstpis => "06 - Operação Tributável a Alíquota Zero";
        public static string CstCofins => "06 - Operação Tributável a Alíquota Zero";
        public static string ClassificacaoPisCofins => "Não monofásica";

        public static string NomeFinalDoProduto => "PRODUTO CODIGO DE BARRAS CADASTRO TESTE";
    }
}
